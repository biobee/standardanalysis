...
```{r, message = FALSE}
input_directory <- "Your/data/directory"
...
You can specify the metadata variables to be parsed as factors in the `factors` argument of `import_data`. Passing no arguments to the function will load the mock dataset.
```{r}
...
Here you can perform a PERMANOVA on all phyloseq objects. `adonis_results` will give you the results for each phyloseq object (Remember that you can comment with "#" to stop a line from being run). You can subset the data you need by providing the `ps_ref` argument with the a subseted `phyloseq` object, OTU table or metadata. The function also works with multiple response variables. Additionally, you can do a longitudinal test by providing the 'longit' argument and assigning your "strata"-variable (PatientID, Location, etc.). Likewise, it is also possible to do PERMANOVA with Aitchison distance ( distance = "aitchison"). In that case, a shift of 1 is applied to each ps-object.

With `permanova_average` you can aggregate the results from `multiple_permanova`. Calling it on the multiple PERMANOVA output (here `adonis_results`) creates an aggregated result dataframe with the median pseudo-F and its IQR (25th-75th percentile) as well as the ACAT p-value. It also plots the pseudo-F statistics and p-values.
```{r}
